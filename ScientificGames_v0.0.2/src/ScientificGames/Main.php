...
class Main extends PluginBase
{
    ...
    public $Step=0;
    public $TITLE = "§7-=§l§dScientificGames§r§7=-";


    public function onEnable()
    {

        $this->getLogger()->info(C::RED.C::BOLD."真实生存插件§1§l---§e§lScientificGames科学游戏 §f§l加载中");
        ...
        if($this->config->get("底部显示格式") != "tip" AND $this->config->get("底部显示格式") != "pop"){
            $this->getLogger()->info(C::RED."底部显示格式未设置或设置错误 §a先已默认为tip格式");
            $this->config->set("底部显示格式","tip");
            $this->config->save();
        }
        $this->getLogger()->info(C::RED.C::BOLD."真实生存插件§1§l---§e§lScientificGames科学游戏 §d§l加载完毕");
    }
//id 时间 等级 开启
    public function ScientificGames_addEffect($player, $id,  $amplifier, $Duration,$visible)
    {
