...
use ScientificGames\Task\Tasker;
...
class Listeners implements Listener
{
        ...
        if($this->plugin->config->get("不能使用指令") == true)
        {
            if(in_array($event->getPlayer()->getLevel()->getFolderName(),$this->plugin->world->get("worlds")))
            {
                ...
                if($cmd == "- cancel")
                {
                    ...
                    $player->sendMessage("§l§a已将你传送回主城   \n    §l----§eScientificGames科学游戏");
                }
            }
        }
    }
    public function onDeath(PlayerDeathEvent $event)
    {
        if(in_array($event->getPlayer()->getLevel()->getFolderName(),$this->plugin->world->get("worlds")))
        {
            ...
            $player->sendMessage("§a你在真实生存世界§l§c[{$level}]§a中死亡了   \n    §l----§eScientificGames科学游戏");
            $level = $this->plugin->getServer()->getDefaultLevel()->getSpawnLocation();
            $player->teleport($level);
            $player->sendMessage("§l§a已将你传送回主城   \n    §l----§eScientificGames科学游戏");
            $this->plugin->tip->remove($name);
    ...
    public function oTeleport(EntityTeleportEvent $event)
    {
        ...
        if($player instanceof Player)
        {
            ...
            if(in_array($level,$this->plugin->world->get("worlds")))
            {
                ...
                $this->plugin->getScheduler()->scheduleRepeatingTask(new CheckTask($this->plugin,$player), 100);
                $this->plugin->getScheduler()->scheduleRepeatingTask(new HightTask($this->plugin,$player), 20*60*60);
                $this->plugin->getScheduler()->scheduleRepeatingTask(new Tasker($this->plugin,$player), 20*60*6);
                $player->sendMessage("§a你来到了§l§c[{$level}]§a真实世界   \n      §l----§eScientificGames科学游戏");
            }
        }
    }
